 ...
 *
 * edited by Becky Gilbert to automatically focus in the first text box
 *
 */
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        if (i === 0) {
          html += '<input autofocus type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'"></input>';
        } else {
        ...
        if (i === 0) {
          html += '<textarea autofocus name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '">'+trial.questions[i].value+'</textarea>';
        } else {
